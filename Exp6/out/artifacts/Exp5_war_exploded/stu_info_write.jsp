        ...
        <div class="login_form">
            <form method="post">
                <ul class="login-item" style="background-color: white">
                    <span>学号:</span>
                    <input type="text" style="border-left:0px;border-top:0px;border-right:0px;border-bottom:1px " name="ID" class="login_input" value="<%out.print(Num);%>" readonly="true">
                </ul>
                <ul class="login-item" style="background-color: white">
                    <span>姓名:</span>
                    <input type="text" name="Name" class="login_input" style="border-left:0px;border-top:0px;border-right:0px;border-bottom:1px ">
                </ul>
                <ul class="login-item" style="background-color: white">
...
<%
    ...
    if(id!=null&&name!=null&&sex!=null&&tel!=null&&class_num!=null&&area!=null){
        String tel_standard="\\d{11}";   //正则表达式验证手机号是否符合要求
        if(tel.matches(tel_standard)){
            String sql2="insert into stu_info(stu_id,stu_name,stu_sex,stu_tel,stu_classno,stu_area) values('"+id+"','"+name+"','"+sex+"','"+tel+"','"+class_num+"','"+area+"')";
            write.write_in(sql2);
        %>
            <script>
            alert('信息录入成功');
